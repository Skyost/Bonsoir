...
class AvahiAddressResolverFound extends DBusSignal {
  ...
  AvahiAddressResolverFound(DBusSignal signal) : super(sender: signal.sender, path: signal.path, interface: signal.interface, name: signal.name, values: signal.values);
}
...
class AvahiAddressResolverFailure extends DBusSignal {
  ...
  AvahiAddressResolverFailure(DBusSignal signal) : super(sender: signal.sender, path: signal.path, interface: signal.interface, name: signal.name, values: signal.values);
}
...
class AvahiAddressResolver extends DBusRemoteObject {
  ...
  AvahiAddressResolver(super.client, String destination, DBusObjectPath path) : super(name: destination, path: path) {
    found = DBusRemoteObjectSignalStream(
      object: this,
      interface: 'org.freedesktop.Avahi.AddressResolver',
      name: 'Found',
      signature: DBusSignature('iiissu'),
    ).asBroadcastStream().map(AvahiAddressResolverFound.new);
    ...
    ).asBroadcastStream().map(AvahiAddressResolverFailure.new);
  }
  ...
  Future<void> callFree({bool noAutoStart = false, bool allowInteractiveAuthorization = false}) async {
    await callMethod('org.freedesktop.Avahi.AddressResolver', 'Free', [], replySignature: DBusSignature(''), noAutoStart: noAutoStart, allowInteractiveAuthorization: allowInteractiveAuthorization);
  }
  ...
  Future<void> callStart({bool noAutoStart = false, bool allowInteractiveAuthorization = false}) async {
    await callMethod('org.freedesktop.Avahi.AddressResolver', 'Start', [], replySignature: DBusSignature(''), noAutoStart: noAutoStart, allowInteractiveAuthorization: allowInteractiveAuthorization);
  }
}
