...
class AvahiEntryGroupStateChanged extends DBusSignal {
  ...
  AvahiEntryGroupStateChanged(DBusSignal signal) : super(sender: signal.sender, path: signal.path, interface: signal.interface, name: signal.name, values: signal.values);
}
...
class AvahiEntryGroup extends DBusRemoteObject {
  ...
  AvahiEntryGroup(super.client, String destination, DBusObjectPath path) : super(name: destination, path: path) {
    stateChanged = DBusRemoteObjectSignalStream(
      object: this,
      interface: 'org.freedesktop.Avahi.EntryGroup',
      name: 'StateChanged',
      signature: DBusSignature('is'),
    ).asBroadcastStream().map(AvahiEntryGroupStateChanged.new);
  }
  ...
  Future<void> callFree({bool noAutoStart = false, bool allowInteractiveAuthorization = false}) async {
    await callMethod('org.freedesktop.Avahi.EntryGroup', 'Free', [], replySignature: DBusSignature(''), noAutoStart: noAutoStart, allowInteractiveAuthorization: allowInteractiveAuthorization);
  }
  ...
  Future<void> callCommit({bool noAutoStart = false, bool allowInteractiveAuthorization = false}) async {
    await callMethod('org.freedesktop.Avahi.EntryGroup', 'Commit', [], replySignature: DBusSignature(''), noAutoStart: noAutoStart, allowInteractiveAuthorization: allowInteractiveAuthorization);
  }
  ...
  Future<void> callReset({bool noAutoStart = false, bool allowInteractiveAuthorization = false}) async {
    await callMethod('org.freedesktop.Avahi.EntryGroup', 'Reset', [], replySignature: DBusSignature(''), noAutoStart: noAutoStart, allowInteractiveAuthorization: allowInteractiveAuthorization);
  }
  ...
  Future<void> callAddService({
    ...
    await callMethod(
      ...
      [
        ...
        DBusString(host),
        DBusUint16(port),
        DBusArray(DBusSignature('ay'), txt.map(DBusArray.byte)),
      ],
  ...
  Future<void> callAddServiceSubtype(
    int interface,
    int protocol,
    int flags,
    String name,
  ...
  Future<void> callUpdateServiceTxt(
    ...
    await callMethod(
      'org.freedesktop.Avahi.EntryGroup',
      'UpdateServiceTxt',
      [DBusInt32(interface), DBusInt32(protocol), DBusUint32(flags), DBusString(name), DBusString(type), DBusString(domain), DBusArray(DBusSignature('ay'), txt.map(DBusArray.byte))],
      replySignature: DBusSignature(''),
      noAutoStart: noAutoStart,
      allowInteractiveAuthorization: allowInteractiveAuthorization,
    );
  ...
  Future<void> callAddAddress(int interface, int protocol, int flags, String name, String address, {bool noAutoStart = false, bool allowInteractiveAuthorization = false}) async {
    await callMethod(
      'org.freedesktop.Avahi.EntryGroup',
      'AddAddress',
  ...
  Future<void> callAddRecord(
    ...
    await callMethod(
      'org.freedesktop.Avahi.EntryGroup',
      'AddRecord',
      [DBusInt32(interface), DBusInt32(protocol), DBusUint32(flags), DBusString(name), DBusUint16(clazz), DBusUint16(type), DBusUint32(ttl), DBusArray.byte(rdata)],
      replySignature: DBusSignature(''),
      noAutoStart: noAutoStart,
      allowInteractiveAuthorization: allowInteractiveAuthorization,
    );
