...
class AvahiRecordBrowserItemNew extends DBusSignal {
  ...
  AvahiRecordBrowserItemNew(DBusSignal signal) : super(sender: signal.sender, path: signal.path, interface: signal.interface, name: signal.name, values: signal.values);
}
...
class AvahiRecordBrowserItemRemove extends DBusSignal {
  ...
  AvahiRecordBrowserItemRemove(DBusSignal signal) : super(sender: signal.sender, path: signal.path, interface: signal.interface, name: signal.name, values: signal.values);
}
...
class AvahiRecordBrowserFailure extends DBusSignal {
  ...
  AvahiRecordBrowserFailure(DBusSignal signal) : super(sender: signal.sender, path: signal.path, interface: signal.interface, name: signal.name, values: signal.values);
}

/// Signal data for org.freedesktop.Avahi.RecordBrowser.AllForNow.
class AvahiRecordBrowserAllForNow extends DBusSignal {
  AvahiRecordBrowserAllForNow(DBusSignal signal) : super(sender: signal.sender, path: signal.path, interface: signal.interface, name: signal.name, values: signal.values);
}
...
class AvahiRecordBrowserCacheExhausted extends DBusSignal {
  AvahiRecordBrowserCacheExhausted(DBusSignal signal) : super(sender: signal.sender, path: signal.path, interface: signal.interface, name: signal.name, values: signal.values);
}
...
class AvahiRecordBrowser extends DBusRemoteObject {
  ...
  AvahiRecordBrowser(super.client, String destination, DBusObjectPath path) : super(name: destination, path: path) {
    itemNew = DBusRemoteObjectSignalStream(
      object: this,
      interface: 'org.freedesktop.Avahi.RecordBrowser',
      name: 'ItemNew',
      signature: DBusSignature('iisqqayu'),
    ).asBroadcastStream().map(AvahiRecordBrowserItemNew.new);
    ...
    itemRemove = DBusRemoteObjectSignalStream(
      ...
      name: 'ItemRemove',
      signature: DBusSignature('iisqqayu'),
    ).asBroadcastStream().map(AvahiRecordBrowserItemRemove.new);
    ...
    ).asBroadcastStream().map(AvahiRecordBrowserFailure.new);
    ...
    ).asBroadcastStream().map(AvahiRecordBrowserAllForNow.new);
    ...
    cacheExhausted = DBusRemoteObjectSignalStream(
      ...
      name: 'CacheExhausted',
      signature: DBusSignature(''),
    ).asBroadcastStream().map(AvahiRecordBrowserCacheExhausted.new);
  }
  ...
  Future<void> callFree({bool noAutoStart = false, bool allowInteractiveAuthorization = false}) async {
    await callMethod('org.freedesktop.Avahi.RecordBrowser', 'Free', [], replySignature: DBusSignature(''), noAutoStart: noAutoStart, allowInteractiveAuthorization: allowInteractiveAuthorization);
  }
  ...
  Future<void> callStart({bool noAutoStart = false, bool allowInteractiveAuthorization = false}) async {
    await callMethod('org.freedesktop.Avahi.RecordBrowser', 'Start', [], replySignature: DBusSignature(''), noAutoStart: noAutoStart, allowInteractiveAuthorization: allowInteractiveAuthorization);
  }
}
